...
(function () {
    ...
    function renderCats() {
        ...
        for (var i = 0; i < CATS.length; i++) {
            ...
            h += '<div data-cat="' + c.id + '" class="flex flex-col items-center gap-1 flex-shrink-0 cursor-pointer" style="min-width:56px">' +
                 '<div class="rounded-full flex items-center justify-center text-xl transition-colors ' + (on ? 'bg-blue-600' : 'bg-slate-100') + '" style="width:52px;height:52px">' + c.emoji + '</div>' +
                 '<span class="text-[10px] font-semibold transition-colors ' + (on ? 'text-blue-600' : 'text-slate-400') + '">' + c.name + '</span></div>';
        }
    ...
    function renderCart() {
        ...
        d.clist.innerHTML = h;
        d.ctotal.textContent = fmt(subTotal());
    }
    ...
    function sendWA() {
        ...
        for (var i = 0; i < o.items.length; i++) t += '- ' + o.items[i].name + ' x' + o.items[i].qty + '\n';
        t += '\nAlamat:\n' + o.address + '\n\nPengiriman:\n' + o.shipping.name + '\n';
